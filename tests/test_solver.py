...
from nncg import kkt_violation, make_problem, make_scaled_problem, solve_nnqp
...
def test_recovers_planted_optimum(kappa: float, seed: int) -> None:
    ...
    assert kkt_violation(a, b, res.x) < 1e-7 * (1.0 + float(np.linalg.norm(b)))
...
def test_max_outer_cap_reports_nonconvergence() -> None:
    ...
    res = solve_nnqp(a, b, max_outer=1)
...
def test_warm_start_survives_support_drift() -> None:
    ...
    cold = solve_nnqp(a, b2)
    warm = solve_nnqp(a, b2, warm=(first.free, first.x))
